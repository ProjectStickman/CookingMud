...
module.exports = {
  listeners: {
    ...
    hit: state => function (damage, target, finalAmount) {
      ...
      var enchantment = damage.source.metadata.enchantment;
      if(enchantment){

      }

      var type = damage.metadata.type;
      var typestr = '';
      if(type){
        var typecolor = 'white';
        switch(type){
          ...
          case 'lightning': typecolor = 'silver'; break;
          case 'earth': typecolor = 'brown'; break;
        }
        typestr = '<'+typecolor+'>'+type+'</'+typecolor+'>';
      }
      let buf = '';
      ...
      buf += ` <b>${target.name}</b> for <b>${finalAmount}</b> ${typestr} damage.`;
